...
DockWindow::DockWindow(QWidget* parent) : QWidget(parent) {
    ...
    // Crear el lanzador de aplicaciones
    m_appMenuLauncher = new AppMenuLauncher(m_scanner, this, this);
    m_appMenuLauncher->show();
...
void DockWindow::layoutItems() {
    ...
    // Tamaño del AppMenuLauncher
    int menuLauncherSize = m_cfg.tamanoIcono + 10;

    if (horizontal) {
        int totalW = pad;
        totalW += menuLauncherSize + esp;  // Espacio para el AppMenuLauncher
        for (DockItem* it : m_items) totalW += it->tamano().width() + esp;
        if (!m_items.isEmpty()) totalW -= esp;
        totalW += pad;
        resize(totalW, grosor);
        
        // Posicionar AppMenuLauncher al inicio (izquierda)
        m_appMenuLauncher->setGeometry(pad, (grosor - menuLauncherSize) / 2, 
                                       menuLauncherSize, menuLauncherSize);
        m_appMenuLauncher->show();
        
        int x = pad + menuLauncherSize + esp;
        for (DockItem* it : m_items) {
        ...
        int totalH = pad;
        totalH += menuLauncherSize + esp;  // Espacio para el AppMenuLauncher
        for (DockItem* it : m_items) totalH += it->tamano().height() + esp;
        if (!m_items.isEmpty()) totalH -= esp;
        totalH += pad;
        resize(grosor, totalH);
        
        // Posicionar AppMenuLauncher al inicio (arriba)
        m_appMenuLauncher->setGeometry((grosor - menuLauncherSize) / 2, pad,
                                       menuLauncherSize, menuLauncherSize);
        m_appMenuLauncher->show();
        
        int y = pad + menuLauncherSize + esp;
        for (DockItem* it : m_items) {
...
void DockWindow::actualizarMagnificacion(const QPoint& mouse) {
    ...
    // Aplicar magnificación también al AppMenuLauncher
    if (m_appMenuLauncher) {
        QPoint centro = m_appMenuLauncher->mapToParent(m_appMenuLauncher->rect().center());
        int dist = horizontal ? abs(mouse.x()-centro.x()) : abs(mouse.y()-centro.y());
        int sz;
        if (dist < rango) {
            double t = 1.0 - (double)dist/rango;
        ...
        m_appMenuLauncher->setTamanoMagnificado(sz);
    }
...
void DockWindow::resetMagnificacion() {
    ...
    if (m_appMenuLauncher) m_appMenuLauncher->setTamanoMagnificado(0);
